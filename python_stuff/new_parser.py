...
NOTES = [TokenType.do, TokenType.re, TokenType.mi, TokenType.fa, TokenType.sol, TokenType.la, TokenType.si,
         TokenType.KW_R]
...
V piano: [
            ...
            if self.peek(0).type == TokenType.KW_TITLE:
                ...
                if self.expect(TokenType.COLON) is None:
                    self.err_list.append(
                        f"{SyntaxErr(self.peek(0))}Expected colon after keyword title, got token \"{self.peek(0).value}\" intead\n")
                    self.restore_stmt()
                else:
                    self.skip_space()
                    ...
                    if name is not None:
                        ...
                        self.err_list.append(
                            f"{SyntaxErr(name)}Expected string after title definiton, got token \"{name.value}\" instead\n")
                        self.restore_stmt()
            ...
            # Handle macro definition or movement
                ...
                if self.peek(0).type in [TokenType.EQUAL, TokenType.OPEN_PAREN]:
                    ...
                    if ident.value in self.idents.keys():
                        if not isinstance(self.idents[ident.value], Token):
                            self.err_list.append(
                                f"{Ident_err(ident)}Identifier {ident.value} is already used here:{self.idents[ident.value]}, redefinitions are not allowed")
                    ...
                    if top != ParseState.MACRO:
                        self.err_list.append(
                            f"Compiler error: Expected Macro state on top of the stack, got {top} intead")
                ...
                # parse movements
                    ...
                    if (m_ident not in self.idents.keys()  # if the movement isn't defined already
                            or self.idents[m_ident][0] != len(self.tracks)
                    # or if defined, doesn't belong to the same track
                    ):
                    ...
                    if top != ParseState.MOVEMENT:
                        self.err_list.append(
                            f"Compiler error: Expected Movement state on top of the stack, got {top} intead")
                ...
                if self.match(TokenType.COLON):
                    ...
                    self.err_list.append(
                        f"{SyntaxErr(self.peek(1))} Expected colon or string after \"track\" , got token \"{self.peek(1).value}\" instead")
    ...
    def parse_macro(self, name):
                ...
                # got some parameters
                    ...
                    if self.match(TokenType.COMMA):
                        ...
                        if self.peek(0).type == TokenType.ALPHANUM:
                            ...
                            self.err_list.append(
                                f"{SyntaxErr(self.peek(0))}Trailing comma in macro definition arguments: \"{self.peek(0).value}\"")
                    ...
                    # if no comma, has to be close paren
                        ...
                        self.err_list.append(
                            f"{SyntaxErr(self.peek(0))}Macro parameter must be fallowed by comma or closed parenthesi, got token \"{self.peek(0).value}\" instead")
                        self.restore_to(TokenType.NL, TokenType.CLOSE_PAREN)
                else:
                    self.err_list.append(
                        f"{SyntaxErr(self.peek(0))}Expected identifier in arguments body, got token \"{self.peek(0).value}\" instead")
        ...
        if not self.match(TokenType.EQUAL):
            self.err_list.append(f"{SyntaxErr(self.peek(0))}Expected equals sign after macro name or arguments",
                                 self.log_tk())
            self.restore_stmt()
    ...
    def parse_expr(self):
        ...
        # Parse SetVolume
            ...
            if self.expect(TokenType.EQUAL) is None:
                self.err_list.append(
                    f"{SyntaxErr(self.peek(0))}Expected colon after keyword {source.value}{self.log_tk()}")
        ...
        # Parse SetMeasure
            ...
            if self.match(TokenType.NUM):
                ...
                if self.expect(TokenType.SLASH) is None:
                    self.err_list.append(
                        f"{SyntaxErr(self.peek(-1))}Measures are defined as number/number, after number got {self.peek(-1)} instead of \"/\"  ")
                    self.restore_to(TokenType.SPACE)
                ...
                if self.match(TokenType.NUM):
                    ...
                    self.err_list.append(
                        f"{SyntaxErr(self.peek(0))}Expected number after defining measure{self.log_tk()}")
                    self.restore_to(TokenType.SPACE)
        ...
        # Parse Repetition
            ...
            if self.match(TokenType.NUM):
                ...
                if self.stack[-1] == ParseState.EXPR:
                    ...
                    self.err_list.append(
                        f"{SyntaxErr(self.peek(0))}Repetition postfix operator can only be used after an expression{self.log_tk()}")
            else:
                self.err_list.append(
                    f"{SyntaxErr(self.peek(0))}Expected number after repetition (*), got '{self.peek(0).value}' instead.")
                self.restore_to(TokenType.SPACE, TokenType.NL, TokenType.EOF)
                self.stack.pop()
                return None
        ...
        # Parse ReleaseNote
            ...
            if self.stack[-1] == ParseState.EXPR:
                ...
                self.err_list.append(
                    f"{SyntaxErr(self.peek(0))}Close paren postfix operator can only be used after an expression{self.log_tk()}")
        ...
        # Parse HoldNote
            ...
            if isinstance(note, Note) or isinstance(note, Chord) or isinstance(note, Ident):
                ...
                self.err_list.append(
                    f"{SyntaxErr(self.peek(0))}Expected note or identifier after open parenthesis{self.log_tk()}")
        ...
        # Parse ident or chord
            ...
            if self.match(TokenType.SLASH):
                ...
                while self.peek(0).type not in [TokenType.NL, TokenType.SPACE]:
                    ...
                    self.err_list.append(
                        f"{SyntaxErr(self.peek(0))}Chords can only be formed from notes,using \"{ident.value}\" is not valid")
                    self.restore_to(TokenType.SPACE)
            ...
            # parse Macro appl
                ...
                while True:
                    ...
                    if self.expect(TokenType.COMMA):
                        ...
                        self.err_list.append(
                            f"{SyntaxErr(self.peek(0))}Expected comma or close parenthesis after expression in argument list",
                            self.log_tk())
        ...
        # Parse Note or chord
            ...
            if self.match(TokenType.SLASH):
                ...
                while self.peek(0).type not in [TokenType.NL, TokenType.SPACE]:
                    ...
                    if isinstance(note, Note):
                        ...
                        self.err_list.append(
                            f"{SyntaxErr(self.peek(0))}Chords can only be formed from notes,using {note} is not valid, notes= {notes}, curr{self.peek(0)}")
            ...
            if self.match(TokenType.COLON):
                ...
                if self.match(TokenType.NUM):
                    ...
                    self.err_list.append(f"{SyntaxErr(self.peek(0))}After colon in note definition expected a number",
                                         self.log_tk())
            ...
            self.err_list.append(
                f"{SyntaxErr(self.peek(0))}Unexpected token while parsing expressions: {self.peek(0).value} ")
    ...
    def restore_to(self, *args):
        while self.peek(0).type not in args and self.peek(0).type != TokenType.EOF:
            self.advance()
